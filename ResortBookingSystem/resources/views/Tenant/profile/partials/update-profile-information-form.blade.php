<section>
    ...
    <form id="send-verification" method="post" action="{{ route('tenant.verification.send', ['tenant_slug' => request()->route('tenant_slug')]) }}">
        @csrf
    </form>

    <form method="post" action="{{ route('tenant.profile.update', ['tenant_slug' => request()->route('tenant_slug')]) }}" class="mt-6 space-y-6">
        @csrf
        @method('patch')
