...
<body class="font-sans text-gray-900 antialiased">
    <div class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0 bg-gray-100 dark:bg-gray-900">
        <div class="w-full sm:max-w-md mt-6 px-6 py-4 bg-white dark:bg-gray-800 shadow-md overflow-hidden sm:rounded-lg">
            <h1 class="text-2xl font-bold mb-6 text-center text-gray-800 dark:text-white">Register Tenant</h1>
            ...
            <form method="POST" action="{{ url('/tenant/register') }}" class="space-y-4">
                @csrf

                <!-- Tenant Name -->
                <div>
                    <label for="tenant_name" class="block font-medium text-sm text-gray-700 dark:text-gray-300">
                        Tenant Name
                    </label>
                    <input id="tenant_name" type="text" name="tenant_name" value="{{ old('tenant_name') }}" 
                           required autofocus
                ...
                <!-- Tenant Slug (used to generate domain) -->
                <div>
                    <label for="slug" class="block font-medium text-sm text-gray-700 dark:text-gray-300">
                        Tenant Subdomain
                    </label>
                    <input id="slug" type="text" name="slug" value="{{ old('slug') }}" 
                           placeholder="djs" required
                           class="block mt-1 w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" />
                    @error('slug')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    @enderror
                ...
                <!-- Register + Already Registered (Aligned Center, Compact, White Button) -->
                <div class="pt-4 flex flex-col items-center space-y-1">
                    ...
                    class="text-sm text-gray-600 dark:text-gray-400
                            hover:text-gray-900 dark:hover:text-gray-100 underline">
                        Already registered?
                    </a>
